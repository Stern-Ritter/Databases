...
where nickname = 'Mildewed';

create index nickname_idx on administration.users(nickname);

drop index if exists administration.nickname_idx;
...
drop index if exists documents.title_search_idx;
...
drop index administration.nickname_email_idx;
...
select 
	...
	gender
from administration.users
where nickname = 'Alierinn'
and email = 'jenny.solberg@hotmail.com';
...
Looking for a complete outdoor cooking setup? Look no further than the Traeger Timberline XL. Featuring an induction cooktop, 1,320 square inches of grill space and a max temperature of 500?F, the Timberline XL ensures that you can create any culinary masterpiece you can imagine. But the Timberlines 
